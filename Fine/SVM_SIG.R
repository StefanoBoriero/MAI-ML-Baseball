    ...
    
  {
    for (coef in 10^seq(0,3))
    {
      ...
      for (j in 1:k){
        ...
        model.sig <- svm(target~., cost=cost, gamma = g, coef0=coef, degree=2, kernel = "sigmoid", data = trainset[,-21])#
        
        # predict TR data
        pred.va <- predict (model.sig, trainset)
        ...
        # predict VA data
        pred.va <- predict (model.sig, newdata=valset)
      ...
      nome <- paste('SIG_cv100_c=',cost,'g=',g,'coef=',coef,'.txt')
      nome2 <- paste('SIG_VA100_error_c=',cost,'g=',g,'coef=',coef,'.txt')
...
write.table(models.recap, file = "SVMS.CSV", sep =" & ", col.names = TRUE)