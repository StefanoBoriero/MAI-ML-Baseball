...
for (cost in 10^seq(-2,3))
{
    ...
    
  {
    for (coef in 10^seq(0,3))
    {
        ...
        
      {
        ...
        model.pol <- svm(target~trainset[,7] + trainset[, 9] + trainset[,10] + trainset[,19], C=cost, gamma = g, coef=coef, degree=d, kernel = "polynomial", data = hitter)
  ...
  nome <- paste('POL_cv_c=',cost,'g=',g,'d=',d,'coef=',coef,'.txt')
  nome2 <- paste('POL_VA_error_c=',cost,'g=',g,'d=',d,'coef=',coef,'.txt')
