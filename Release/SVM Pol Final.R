    ...
    for (coef in 10^seq(0,3))
    {
      ...
      for (d in seq(2,3))
      {
        ...
        for (j in 1:k){
          ...
          model.pol <- svm(target~., cost=cost, gamma = g, coef0=coef, degree=d, kernel = "polynomial", data = trainset[,-17])
        ...
        nome <- paste('POL_cv100_c=',cost,'g=',g,'d=',d,'coef=',coef,'.txt')
        nome2 <- paste('POL_VA100_error_c=',cost,'g=',g,'d=',d,'coef=',coef,'.txt')
