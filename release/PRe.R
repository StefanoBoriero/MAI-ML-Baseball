...
#Hitter666 <- data.frame(cbind(scale(Hitter[,1:20]), scale(Hitter[,22:23]),target.log))
Hitter666 <- data.frame(cbind(scale(Hitter$Nputouts), scale(Hitter$Nwalks), scale(Hitter$Nhits), scale(Hitter$Nbat_career), scale(Hitter$Nhits_career), scale(Hitter$Nruns_career), target.log))
...
names(Hitter666)[7] <- 'Annual_salary'



#colnames(Hitter[,21]) <- c("Log_Salary_1987")
write.csv(Hitter666, file="hitter_scaled_log_Final.csv", row.names = FALSE)
