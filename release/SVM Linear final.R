...
hitter <- read.csv("hitter_scaled_log_Final.CSV", header=TRUE, sep =",")
library(kernlab)
...
for (cost in 10^seq(-2,3)){
  ...
  for (j in 1:k){
    ...
    model.linear <- svm(target~.,  cost=cost, kernel = "linear", data = trainset[,-7])#
  ...
  nome <- paste('LIN_cv100_c=',cost,'.txt')
  nome2 <- paste('LIN_VA100_error_c=',cost,'.txt')
...
write.table(models.recap, file = "SVML.CSV", sep =" & ", col.names = TRUE)
